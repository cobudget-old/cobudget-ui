    ...
    Object.defineProperties scope.model,
      myContributionAmount:
        ...
        set: (amount) ->
          ...
          if not myContribution.amount.eq(0) and bucket.myContributionIndex == -1
            bucket.contributions.add(myContribution)
      ...
      startContributing:
        value: ($event) ->
          ...
          # focus onto input after rendering unsaved state
          $timeout ->
            input = element.find('input[type="number"]')

            # HACK for firefox micro-blurring on focus
            scope.model.focusHack = true
            debug("focusing input", input)
            input.focus() if not input.is(':focus')
            scope.model.focusHack = false

      saveContribution:
        value: (myContribution) ->
          debug("saveContribution", myContribution)

          # hack for firefox micro-blurring on startContributing focus
          return if scope.model.focusHack
